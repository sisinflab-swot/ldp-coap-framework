...
//		if (Bench_Help.DO_LOG) 
			...
			//number.getAndIncrement();//incremento il numero di risposte giunte
...
//				if (Bench_Help.DO_LOG) 
	...
	private final class CoapResponseWorker extends Thread {
		 ...
		 */
		public CoapResponseWorker(String name, Request coapRequest, HttpAsyncExchange httpExchange, HttpRequest httpRequest, Thread responseWorker, HttpResponse httpResponse, String type) {
			super(name);
			this.coapRequest = coapRequest;
...
//				if (Bench_Help.DO_LOG) 
			...
			try {
				...
				if(type.equals("standard")){
					...
					HttpTranslator.getHttpDiscovery(httpRequest, coapResponse, httpResponse);
					
					//Stampo informazioni a video
					System.out.println("DISCOVERY OUTPUT:");
					System.out.println(coapResponse.toString());
					log = log + "DISCOVERY OUTPUT:\n";
					log = log + coapResponse.toString() + "\n";
					String payload = coapResponse.getPayloadString();
					if(payload!=null && payload!=""){
						System.out.println(payload);
					...
					HttpTranslator.getHttpOptions(httpRequest, coapResponse, httpResponse);
					
					//Stampo informazioni a video
					System.out.println("OPTIONS OUTPUT:");
					System.out.println(coapResponse.toString());
					log = log + "OPTIONS OUTPUT:\n";
					log = log + coapResponse.toString() + "\n";
					String payload = coapResponse.getPayloadString();
					if(payload!=null && payload!=""){
						System.out.println(payload);
...
//					if (Bench_Help.DO_LOG) 
					...
					if(httpRequest.getRequestLine().getMethod().toLowerCase().equals("get")){
						
						//Stampo informazioni a video
						System.out.println("GET STANDARD REQUEST:");
						System.out.println(coapRequest.toString());
						log = log + "GET STANDARD REQUEST:\n";
						log = log + coapRequest.toString() + "\n";
						
						// fill the maps
						exchangeMap.put(coapRequest, new Exchanger<Response>());

						Request discovery = HttpTranslator.createCoapRequestDiscovery(coapRequest.getOptions().getProxyUri());
						Request options = HttpTranslator.createCoapRequestOptions(coapRequest.getOptions().getProxyUri());
						
						//Stampo informazioni a video
						System.out.println("GET/DISCOVERY REQUEST:");
						System.out.println(discovery.toString());
						System.out.println("GET/OPTIONS REQUEST:");
						System.out.println(options.toString());
						log = log + "GET/DISCOVERY REQUEST:\n";
						log = log + discovery.toString() + "\n";
						log = log + "GET/OPTIONS REQUEST:\n";
						log = log + options.toString() + "\n";
						
						exchangeMap.put(discovery, new Exchanger<Response>());
						exchangeMap.put(options, new Exchanger<Response>());
						...
						Thread responseWorker = new CoapResponseWorker("HttpStack Worker: producer", coapRequest,httpExchange, httpRequest, requestWorker, httpResponse, "standard");
						
						//Istanzio i thread per gestire la richiesta di discovery
						Thread requestWorkerDisc = new CoapRequestWorker("HttpStart Worker: discovery consummer", discovery);
						Thread responseWorkerDisc = new CoapResponseWorker("HttpStack Worker: producer", discovery ,httpExchange, httpRequest, requestWorkerDisc, httpResponse, "discovery");
						
						//Istanzio i thread per gestire la richiesta options
						Thread requestWorkerOpt = new CoapRequestWorker("HttpStart Worker: options consummer", options);
						Thread responseWorkerOpt = new CoapResponseWorker("HttpStack Worker: producer", options ,httpExchange, httpRequest, requestWorkerOpt, httpResponse, "options");
						...
						requestWorkerDisc.start();
						responseWorkerDisc.start();
						try {
							responseWorkerDisc.join();
						} catch (InterruptedException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
						
						requestWorkerOpt.start();
						responseWorkerOpt.start();
						try {
							responseWorkerOpt.join();
						} catch (InterruptedException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}		
						...
						//httpExchange.submitResponse();
					} else if(httpRequest.getRequestLine().getMethod().toLowerCase().equals("post")){
						
						//Stampo informazioni a video
						System.out.println("POST STANDARD REQUEST:");
						System.out.println(coapRequest.toString());
						log = log + "POST STANDARD REQUEST:\n";
						log = log + coapRequest.toString() + "\n";
						String payload = coapRequest.toString();
						if(payload!= null && payload!=""){
							System.out.println(payload);
							log = log + payload + "\n";
						}
						...
						//httpExchange.submitResponse();
					} else if(httpRequest.getRequestLine().getMethod().toLowerCase().equals("put")){
						
						//Stampo informazioni a video
						System.out.println("PUT STANDARD REQUEST:");
						System.out.println(coapRequest.toString());
						log = log + "PUT STANDARD REQUEST:\n";
						log = log + coapRequest.toString() + "\n";
						String payload = coapRequest.toString();
						if(payload!= null && payload!=""){
							System.out.println(payload);
						...
						//httpExchange.submitResponse();
					} else if(httpRequest.getRequestLine().getMethod().toLowerCase().equals("patch")){
						//Stampo informazioni a video
						System.out.println("PATCH STANDARD REQUEST:");
						System.out.println(coapRequest.toString());
						log = log + "PATCH STANDARD REQUEST:\n";
						log = log + coapRequest.toString() + "\n";
						String payload = coapRequest.toString();
						if(payload!= null && payload!=""){
							System.out.println(payload);
					...
					} else if(httpRequest.getRequestLine().getMethod().toLowerCase().equals("delete")){
						
						//Stampo informazioni a video
						System.out.println("DELETE STANDARD REQUEST:");
						System.out.println(coapRequest.toString());
						log = log + "DELETE STANDARD REQUEST:\n";
						log = log + coapRequest.toString() + "\n";
						String payload = coapRequest.toString();
						if(payload!= null && payload!=""){
							System.out.println(payload);
						...
						//httpExchange.submitResponse();
					}else if(httpRequest.getRequestLine().getMethod().toLowerCase().equals("options")){
						
						// create the http response and set the status line
						String reason = EnglishReasonPhraseCatalog.INSTANCE.getReason(204, Locale.ENGLISH);
						StatusLine statusLine = new BasicStatusLine(HttpVersion.HTTP_1_1, 204, reason);
						httpResponse.setStatusLine(statusLine);
						...
						//httpExchange.submitResponse();
					} else if(httpRequest.getRequestLine().getMethod().toLowerCase().equals("head")){
						
						String proxyUri = coapRequest.getOptions().getProxyUri();
						coapRequest.getOptions().setProxyUri(proxyUri+"?ldp=head");
						
						//Stampo informazioni a video
						System.out.println("GET/HEAD STANDARD REQUEST:");
						System.out.println(coapRequest.toString());
						log = log + "GET/HEAD STANDARD REQUEST:\n";
						log = log + coapRequest.toString() + "\n";
						
						// fill the maps
						exchangeMap.put(coapRequest, new Exchanger<Response>());
						
						Request discovery = HttpTranslator.createCoapRequestDiscovery(proxyUri);
						Request options = HttpTranslator.createCoapRequestOptions(proxyUri);
					...
					if(httpResponse.getEntity() != null) {
						...
						try {
							...
							if (payload != null && payload.length > 0) {
								String value = new String(payload, "UTF-8");
								//HttpEntity newhttpEntity = new StringEntity(value); //+ "\n" + log);
						...
						//HttpEntity newhttpEntity = new StringEntity(log);
						//httpResponse.setEntity(newhttpEntity);
					}
